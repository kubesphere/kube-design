...
import { get, isEmpty, isUndefined, isFunction, isArray, isString } from "lodash";
...
export interface SelectProps {
  ...
  onFetch: (value: Partial<PaginationProps> & Partial<{ more: boolean }>) => void,
  onChange: (value?: Value) => void,
  optionRenderer?: (option: SelectOption) => React.ReactNode,
  valueRenderer?: (value: SelectOption) => React.ReactNode,
}
...
export default class Select extends React.Component<SelectProps, SelectState> {
  ...
  state = {
    visible: false,
    value: this.props.multi ? [] as string[] : "",
    inputValue: "",
    inputVisible: true,
  };
  ...
  optionsRef = React.createRef<HTMLDivElement>();
  inputValueRef = React.createRef<HTMLSpanElement>();
  scrollParents: any[] = [];
  reachBottom = false;
    ...
    if (
      ...
      if (scrollParent === window || scrollParent === document) {
        ...
        if (!hasRoomBelow && hasRoomAbove) {
          this.optionsRef.current.style.top = `${controlRect.top - optionsRect.height - optionsMargin * 2
            }px`;
        }
      } else {
  ...
  updateInputDOM = ({ width, focus }: { width: string, focus?: boolean }) => {
  ...
  handleOptionClick = (option: SelectOption) => {
    ...
    const selectValue = option.value as string;
  ...
  handleOptionsOpen = (e: React.SyntheticEvent<HTMLElement>) => {
    const { visible } = this.state;
  ...
  handleOptionsScroll = (e: React.SyntheticEvent<HTMLDivElement>) => {
    const { onFetch, pagination } = this.props;
    if (this.reachBottom || !onFetch || !pagination) {
      return;
    }
  ...
  handleDocumentClick = (e: Event) => {
    ...
    if (visible && this.selectRef && this.selectRef.current) {
      if (!this.selectRef.current.contains(e.target as Element)) {
        this.handleOptionsClose();
      }
    }
  };

  handleInputStatus = (visible: boolean) => {
    const { value, inputValue } = this.state;
    const { multi, searchable, options } = this.props;
    const option = (options.find((item) => item.value === value) || {}) as SelectOption;
  ...
  handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newState = { inputValue: value } as SelectState;
    if (!multi) {
  ...
  handleMultiValueDelete = (e: React.SyntheticEvent<HTMLSpanElement>, i: number) => {
  ...
  handleClearValue = (e: React.SyntheticEvent<HTMLElement, Event>) => {
    e.nativeEvent.stopImmediatePropagation();
    e.stopPropagation();
  ...
  renderIcon = () => {
    ...
    return (
      <span
        ...
        onClick={disabled ? undefined : this.handleOptionsOpen}
      >
  ...
  renderPrefixIcon = (icon: React.ReactNode) => {
    return icon ? (
  ...
  renderOptions = () => {
    ...
    if (!visible || !get(this.selectRef, "current") || !this.selectRef.current) {
      return null;
    }
    ...
    return (
      <div
        ...
        ref={this.optionsRef}
        onScrollCapture={canFetch ? this.handleOptionsScroll : undefined}
      >
  ...
  renderOption = (options: SelectOption[]) => {
    const { value } = this.state;
    const { multi, optionRenderer } = this.props;
    const isActive = (v: SelectOption) =>
      multi && isArray(value) ? value.includes(String(v.value)) : v.value === value;
  ...
  renderMultiValue = (value: string | number, i: number) => {
  ...
  renderBaseValues = () => {
    ...
    if (isString(value)) {
      let option: SelectOption = { label: value, value };
      options.forEach((item) => {
        if (item.value === value) {
          option = item;
        } else if (item.options) {
          item.options.forEach((op: SelectOption) => {
            if (op.value === value) {
              option = op;
            }
          });
